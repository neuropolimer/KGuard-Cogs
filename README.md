...
Можно добавить целую категорию в исключения. В ней MuteGuard вообще не накладывает свои запреты на заглушенных участников, но и не выдаёт им `View Channel`, `Send Messages` или другие разрешения. Доступ к такой категории по-прежнему определяется обычными ролями Discord:

```text
[p]muteguardset allowcategory <категория>
[p]muteguardset denycategory <категория>
[p]muteguardset categories
```

Это подходит для отдельной категории вроде «мут-чата»: роль доступа к ней можно выдать самому участнику, а MuteGuard не будет мешать общению внутри неё.
